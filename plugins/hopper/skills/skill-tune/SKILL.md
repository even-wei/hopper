---
name: skill-tune
description: Use to tune a Claude Code SKILL.md (or any agent skill markdown file) against recent transcripts using SkillOpt-style bounded edits. Ports the SkillOpt paper's textual learning rate, atomic add/insert_after/replace/delete operations, failure+success dual analysis with hierarchical merge, per-skill rejected-edit buffer, and protected slow-update region into a human-in-the-loop workflow where you act as the held-out validation gate. Triggers on "tune this skill", "improve this skill", "skill-tune", "skillopt", "refine SKILL.md from transcripts", "apply SkillOpt to <skill>".
---

# Skill Tune — Single-Session SkillOpt

Mirrors the SkillOpt training loop ([arXiv:2605.23904v2](https://arxiv.org/abs/2605.23904), Microsoft Research, May 2026) at single-session, single-skill scale. The target SKILL.md is the trainable state; rollout evidence comes from recent Claude Code transcripts or pasted friction moments; the textual learning rate `L` bounds how many edits land per pass; you act as the held-out validation gate (each edit is accepted or rejected by hand); rejected edits accumulate in a per-skill buffer that informs future passes; an opt-in slow-update step writes longitudinal guidance into a protected region of the skill.

The Codex-readable canonical source is `hopper/prompts/skill-tune-task.md`. The full prompt body is inlined below for Claude Code skill consumers, followed by the rejected-edit buffer schema and provenance so the skill is self-contained regardless of where it's invoked.
...
- Append rejected edits to `<skill-dir>/_rejected_edits.md`, one block per edit, schema in the appendix below. Create the file with a header if it does not exist.
...
## Rejected Edits Buffer Schema

Path: `<skill-dir>/_rejected_edits.md`. Append-only log. Each rejected edit becomes one block. Newest entries at the bottom.

````markdown
# Rejected Edits — <skill name>

Persistent buffer for skill-tune rejected proposals. Each block records one rejected edit so future tune passes can avoid re-proposing the same change.

---

## YYYY-MM-DD HH:MM — rejected

- **op**: `insert_after | replace | append | delete`
- **target**: `<exact anchor text, if applicable>`
- **source_type**: `failure | success`
- **support_count**: `N`
- **cited_moments**: `moment#1, moment#3`
- **reason**: `too-vague | duplicate | instance-specific | wrong-target | would-break-existing | <free text>`

**content**:
```
<the markdown the optimizer proposed>
```
...
This skill is a single-session, human-in-the-loop port of **SkillOpt** (Yang et al., Microsoft Research, May 2026 — [arXiv:2605.23904v2](https://arxiv.org/abs/2605.23904), code at [aka.ms/SkillOpt](https://aka.ms/SkillOpt), MIT license). The paper trains a `best_skill.md` for a frozen LLM agent using rollout batches, an external optimizer model, bounded edit budgets, validation gates, rejected-edit buffers, and epoch-wise slow/meta updates. SkillOpt is best or tied-best on 52 of 52 (model, benchmark, harness) cells evaluated in the paper, and lifts GPT-5.5 by +23.5 (direct chat), +24.8 (Codex), +19.1 (Claude Code) points over no-skill baselines.

This skill ports only the mechanics that survive the human-as-validation-gate translation:
- **Bounded atomic edits** (paper §3.4) — `append`, `insert_after`, `replace`, `delete` only, capped at L.
- **Failure + success dual analysis with failure-priority merge** (paper §3.3, Appendix C.2.1–C.2.5).
- **Per-skill rejected-edit buffer** (paper §3.5) — negative feedback for future passes.
- **Protected slow-update region** (paper §3.6) — fenced by `<!-- SLOW_UPDATE_START -->` / `<!-- SLOW_UPDATE_END -->`, only writable by Stage 7.
- **Validation gate** (paper §3.5) — but the held-out eval split is replaced by per-edit human review; ties reject.

Skipped vs the paper: rollout harness, automated scoring, held-out test split, optimizer-side meta skill, learning-rate schedules. Hopper has no eval infrastructure, so the loop relies on the human to decide whether a candidate edit is an improvement. The paper's "training cost is paid once before deployment" property still holds — once tuned, the skill is just markdown text that runs at zero extra inference cost.
